...
- **轴序**：dataset 的 image 与 label 都直接取 nibabel `[:, :, z]`；推理画布也按
  `[:, :, z]` 写回。`load_label_volume` 必须保持原始轴序，不能转置前两维。
  旧实现错误转置 GT；方形病例 shape 不变却逐像素错位，使验证 Dice 严重失真。
...
- 整卷推理：`prob/pred (512,512,135)` 与 `GT` 同形状，`pad_offset=(0,0)`；
  **GT 前景 434721 体素 = `data/splits.json` 里 case 33 的 434721 mm³**（1mm³/体素）——
  这只能证明肿瘤体素总数与形状合理，不能证明预测与 GT 的逐像素坐标一致；
  现在由 `src.selfcheck_data` 对验证病例的阳性层做逐像素对齐检查。
...
- **执行顺序**：中心层与邻居层先补边并叠成 `(3,H,W)`，然后对整块与 label 施加
  同一套几何增强。所以：
  * 三个通道的翻转、旋转与仿射位置一致；
  * gamma / 高斯噪声只动**被监督的中心通道**；未增强样本的上下文仍可与相邻层单独读取结果
    逐像素核对，增强后的上下文则会随几何变换同步移动；
