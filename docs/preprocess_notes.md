...
| 病灶级检出（原判据） | 逐 GT 病灶判：**与预测有任意重叠** 或 **该 GT 病灶 ≥ `eval.detect_min_mm3`（默认 10 mm³）** 即算检出。⚠️ 见下面「恒真问题」 |
| **病灶覆盖质量（主口径）** | 每病灶 `overlap_frac = 交集体素 / 该 GT 病灶体素`（= 病灶级灵敏度，模型撒得再大也不会被撑到 1）；报 `mean/median_overlap_frac`、`n_covered`（≥0.20）、`n_covered_strict`（≥0.50）与 5 档直方图 `[0,0.1)/[0.1,0.25)/[0.25,0.5)/[0.5,0.75)/[0.75,1]` |
| 假阳性 | 两种口径都给：落在 GT 之外的**体素**总量，以及**与 GT 零重叠的连通块**（个数 / 体积 / 最大块）。5 例仅肝脏病人的 GT 全空，两者分别等于预测总体积与预测病灶总数 |
| 汇总 | **macro（逐例等权）与池化（所有体素合并后再算比率）两种都给**；`std` 用样本标准差（ddof=1，4 例/折时总体标准差会系统性偏小）；另给逐例 `best_cover`（该例覆盖得最好的病灶比例），与 Dice 并排可区分「没找到」与「找歪了」 |
| 报告主表 | 用**后处理之后**的数字；后处理**之前**的同一组指标写进 JSON（`voxel_raw` / `lesion_raw` / `fp_raw`），用于回答「删块值多少 Dice / 召回」 |

**恒真问题（第 5 轮用 `runs/smoke_fold0_fixed` 实测撞到，必须记住）**：原判据的第二条
「该 GT 病灶 ≥ 10 mm³ 即算检出」在本数据集上**恒真** —— 20 例含肿瘤病人的肿瘤体积下限是
652 mm³（case 59，见 `data/splits.json`，全部 19 个 val 病灶都 ≥ 652 mm³）。
于是 fold 0 报出 **19/19 = 1.000**，而同一批病例里 57/59 的体素级 Dice 明明是 **0.0000**
（模型撒出 20k–23k mm³、跟 4k/652 mm³ 的病灶零重叠）——**这一列当时没有任何信息量**。
现在报告会显式打 `criterion_b_is_trivial` 警告，并以下面两条为主：

- **覆盖比例分档表**（`by_overlap_frac`）：多少病灶被真正覆盖住（≥0.5 才算标出来）；
- **逐例 `best_cover` + Dice 并排**：`best_cover` 高而 Dice 低 ⇒ 位置找对了但撒得太大；
  两者都低 ⇒ 根本没找到。

动机（`docs/todo.md` 第 5 轮）：肿瘤体积跨 3 个数量级，fold 0 上 case 33（434 721 体素）能到 0.76
而 57（4 046）/59（652）长期 0 ⇒ 任何「只看均值」的报告都会掩盖小病灶的系统性失败，
所以逐例值、覆盖分档、仅肝脏 FP 三条必须与均值一起报。
